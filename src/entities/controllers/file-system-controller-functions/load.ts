...
export class FSReadFunctions {
  ...
  private static async importVersion (parentProject : Project, version : string) : Promise<SystemConfiguration> {
  ...
  public static async project (projectName : string) : Promise<Project> {
  ...
  public static async configuration (parentProject : Project,  versionString : string) : Promise<void> {
  ...
  public static async projectConfigurations (project : Project) : Promise<void> {
  ...
  public static async projectAndVersions (projectName : string) : Promise<void> {
    const project = await this.project(projectName);
    await this.projectConfigurations(project);
  }

  public static async projects () : Promise<void> {
    const availableProjects = await this.getProjectsList();
    for(const project of availableProjects) await this.project(project);
  }

  public static async projectsAndVersions () : Promise<void> {
    const availableProjects = await this.getProjectsList();
    for(const project of availableProjects) await this.projectAndVersions(project);
  }
}
