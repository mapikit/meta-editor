<script lang="ts">
  import { DivScreenOperations } from "../../lib/div-screen-operations";
...
<div class='bg-norbalt-200 shadow border border-norbalt-100 absolute
  my-2 {expandDirection === "bottom" ? "top-full" : "bottom-full"}
  rounded text-sm left-0 overflow-y-auto max-h-[40vh] z-[1] whitespace-nowrap
  flex flex-col'
  bind:this={listElement}
  style="height: {heightStyle}"
>
  {#each options as option }
    <button class="px-2 py-1 first:pt-2 last:pb-2 hover:bg-norbalt-100 transition-all duration-100"
      on:click="{() => {field = option.value; onChange(option); }}"> {option.label} </button>
  {/each}
</div>
