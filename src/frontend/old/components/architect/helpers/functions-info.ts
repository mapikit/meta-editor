...
import { businessOperations, currentConfig, schemas } from "../../../stores/configuration-store";
...
export class FunctionsInfo {
  ...
  public static getCardInfo (moduleConfig : ModuleCard) : FunctionDefinition {
    switch (moduleConfig.moduleType) {
      ...
      case "protocol":
        return FunctionsInfo.getProtocolsInfo(moduleConfig);
      default:
        return undefined;
    }
  }
  ...
  private static getProtocolsInfo (module : ModuleCard) : FunctionDefinition {
    const protocol = get(currentConfig).protocols.find(_protocol => get(_protocol.identifier) === module.modulePackage);
    const functionInfo = protocol.definition.functionDefinitions.find(
      (func : FunctionDefinition) => func.functionName === module.moduleName);
    return functionInfo as FunctionDefinition ?? undefined;
  }
