...
<div class="absolute h-[calc(100%_+_0.25rem)] bg-norbalt-500 text-center pr-1 pl-2 rounded shadow-[0_0_0_1px_rgba(255,255,255,0.2)] text-xs  top-[50%] -translate-y-[50%] left-0 -translate-x-[calc(100%_+_0.5rem)] flex flex-row items-center overflow-visible"
  on:mouseenter={toggleIsOver}
  on:mouseleave={toggleIsOver}
  on:click={removeConnection}
>

  <Tooltip visible={isOverElement} tooltipContent={`Variable (${variableData?.type}): ${valueDisplay}`} position="top"/>
