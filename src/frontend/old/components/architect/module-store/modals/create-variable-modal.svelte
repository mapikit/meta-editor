<script lang="ts">
  ...
  import BooleanField from "../../../object-definition/editing-fields/boolean-field.svelte";
  ...
  import type { BopsVariable } from "meta-system/dist/src/configuration/business-operations/business-operations-type";
  ...
  let openObjectModal = () : void => undefined;
  let closeObjectModal = () : void => undefined;

  const confirmVariable = () : void => {
    const newValue = { name: get(name), type: get(type), initialValue: get(value) } as BopsVariable;

    if (hasInitial) {
      currentBop.updateVariable(initialName, newValue);
      storeModalOpen.set(false);
      return;
    }

    currentBop.variables.update(vars => {
      vars.push(newValue);
      return vars;
    });
    storeModalOpen.set(false);
...
<div class="flex flex-row justify-between">
  <div class="text-white font-bold text-lg"> {hasInitial ? "Edit" : "Create New"} Variable </div>
...
{#if $type === "object"}
  ...
  > Set Value </div>
{:else if $type === "string"}
  <TextField field={value} label="InitialValue" placeholder=""/>
{:else if $type === "number"}
  <div class="mt-2">
    <p class="text-offWhite text-sm mb-1"> InitialValue </p>
    <input
...
<Modal bind:closeModal={closeObjectModal} bind:openModal={openObjectModal} >
  <div class="flex flex-col" on:click={(e) => { e.stopPropagation(); }}>
    <div class="flex flex-row justify-between">
      <p class="text-lg font-bold mb-3"> Set Variable "{$name}" value </p>
