<script lang="ts">
  ...
  function getPackage () : string {
    switch (moduleType) {
      ...
      case "protocol": return module.protocolIdentifier;
      case "variable": return module.schemaName; // Using this name for this case :/
  ...
  const getModuleDragData = () : ModuleCardType => {
    return ModuleCardType.generate({
      key: undefined, // Will be defined once, on drop
...
<Draggable style={"w-full mt-4 first:mt-0"} dragType="module" dragElement={ref} dragData={getModuleDragData()}>
