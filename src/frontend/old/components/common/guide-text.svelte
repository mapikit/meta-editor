...
<div class="fixed font-sans text-3xl font-black select-none right-9 top-28">
  {#key currentText}
    <h1 class="absolute right-0 top-0 w-[550px] text-right text-norbalt-100" in:customFly|global="{{ x: -50, duration: 460, delay: 160 }}" out:customFly|global="{{ x: -50, duration: 150 }}">
      {currentText}
    </h1>
  {/key}
</div>
