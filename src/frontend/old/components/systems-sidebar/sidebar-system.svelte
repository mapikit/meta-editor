<script lang="ts">
  ...
  let id = system.id;
  $: versions = $availableConfigurations.filter((elm) => get(elm.projectId) === $id);
  ...
  $: currentVersion = versions && versions[0];
  ...
  const goToConfiguration = () : void => {
    navigation.navigateTo(`/editor/system/${$id}/configuration/${get(currentVersion.id)}`);
  };

  const goToManage = () : void => {
    navigation.navigateTo(`/editor/system/${$id}/manage`);
  };
