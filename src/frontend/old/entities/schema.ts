...
export class Schema {
  ...
  public getCardInfo (versionContext : Configuration) : PropertyListEntry {
    const protocol = versionContext.protocols.find(prot => get(prot.identifier) === get(this.dbProtocol));
    return {
      type: "Schema",
      id: get(this.id),
      name: this.name,
      ...
      dataValues: [
        { name: "DB Protocol", value: protocol?.protocolName ?? writable("---"), editable: true },
      ],
    };
  }
