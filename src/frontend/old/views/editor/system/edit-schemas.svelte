<script lang="ts">
  ...
  $: protocolsOptions = $protocols.filter(protocol => get(protocol.protocolKind) === "dbProtocol").map((protocol) => ({
    value: get(protocol.identifier),
    label: `${get(protocol.protocolName)} (${get(protocol.protocol)} v${get(protocol.protocolVersion)})`,
  }));
  ...
  function saveSchema () : void {
    void storageManager.manager.updateSchema(versionId, get(currentSchema))
  ...
  function updateDbProtocol (seleceted :  { label : string, value : string }) : void {
    const value = seleceted.value;
    if(value === undefined) throw Error("Got undefined db protocol");
    editted = true;
    dbprotocol.set(value);
  }
</script>
    ...
    <div class="flex flex-row w-full mt-4"> <!-- Card holder -->
      <div class="rounded bg-norbalt-200 p-3 px-5 border-transparent border w-[550px]">
        ...
        <div class="mt-2 w-full">
          ...
          <div class="mt-1">
            <Selector field={$dbprotocol} options={protocolsOptions} selectedLabel={selectedProtocolLabel} onChange={updateDbProtocol}/>
          </div>
        </div>
