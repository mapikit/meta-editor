<script lang="ts">
  ...
  import PopupRepositioner from "src/frontend/components/popup-repositioner.svelte";
...
>
  ...
  <div class="absolute top-0 left-0.5 h-full w-full">
    {#if open}
    <PopupRepositioner >
      <!-- svelte-ignore a11y-no-static-element-interactions -->
      <div class="fixed rounded bg-norbalt-300 overflow-hidden px-2 py-1 pb-2 shadow-lg
      flex flex-col justify-start items-center z-10"
  ...
  <button class="w-5 h-5 rounded-sm hover:text-white
  hover:bg-norbalt-200 transition justify-center items-center flex
  {open ? "text-white bg-norbalt-200" : "bg-transparent text-offWhite"}"
